...
tsk_adult = tsk("adult_train")
tsk_adult$filter(rows = sample(tsk_adult$row_ids, 1000))
lrn_ranger = lrn("classif.ranger", predict_type = "prob")
lrn_ranger$train(task = tsk_adult)
rsmp_cv5 = rsmp("subsampling", repeats = 2L, ratio = 0.5)
rr_7 = resample(tsk_adult, lrn_ranger, rsmp_cv5, store_model = TRUE)
summary(lrn_ranger, rr_7, summary_control(importance_measures = NULL, complexity_measures = NULL, effect_measures = NULL))
```
